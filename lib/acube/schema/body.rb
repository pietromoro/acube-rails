  ...
  module Schema
    class Body
      ...
      class Item
          ...
          Nokogiri::XML::Builder.new do |xml|
            xml.DettaglioLinee {
              ...
              if vat_exemption
                ...
                xml.PrezzoUnitario ("%.2f" % price.to_f)
                xml.PrezzoTotale ("%.2f" % price.to_f)
                xml.AliquotaIVA ("0.00")
                xml.Natura (vat_exemption.to_s)
              else
                xml.PrezzoUnitario ("%.2f" % unitary_price.to_f)
                xml.PrezzoTotale ("%.2f" % price_no_vat.to_f)
        ...
        def price_no_vat
          price / ((100 + (ACube.vat_amount * 100)) / 100)
        end
      ...
      def self.from(invoice)
        new.tap do |body|
          ...
          raise "Unknown items" unless body.items.is_a?(Array)
          raise "At least one item is required" unless body.items.length > 1

          body.items.map!.with_index do |item, idx|
            raise "Unknown item format" unless item.is_a?(Hash)
          ...
          xml.FatturaElettronicaBody {
            ...
            xml.DatiBeniServizi {
              ...
              items.each_with_object(Hash.new {|h, k| h[k] = [0, 0]}) do |item, h|
                h[t.vat][0] += item.vatable_price 
                h[t.vat][1] += item.vat_amount 
